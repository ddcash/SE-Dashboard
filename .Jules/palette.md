...
## 2026-07-18 - Dynamic Empty State Announcement
**Learning:** When content is dynamically replaced with an empty state (like 'No results found'), screen readers may remain silent, leaving visually impaired users unaware of the state change.
**Action:** Always add `aria-live="polite"` to dynamic empty state containers so screen readers automatically announce their appearance without interrupting the user.
