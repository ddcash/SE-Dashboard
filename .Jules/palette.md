...
## 2026-07-16 - Dynamic Empty State Accessibility
**Learning:** In the bookmark-manager app, dynamic empty state containers must include the `aria-live="polite"` attribute to ensure they are announced by screen readers.
**Action:** Always add `aria-live="polite"` to dynamically rendered `.empty-state` containers.
