...
## 2026-07-23 - Accessibility for Empty States
**Learning:** Dynamic empty states appearing after user actions (e.g., search) are often missed by screen readers if they are not explicitly marked as live regions.
**Action:** Always add `aria-live="polite"` to empty state containers so that assistive technologies announce the lack of results to the user.
