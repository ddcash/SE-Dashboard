...
## 2026-07-16 - Dynamic Empty States Accessibility
**Learning:** Dynamic empty state containers are often inserted into the DOM without notifying screen reader users, causing them to be unaware of the state change (like empty search results).
**Action:** Always add the `aria-live="polite"` attribute to dynamic empty state containers (e.g., `.empty-state`) to ensure they are announced by screen readers when they appear in the UI.
