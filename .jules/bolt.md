...
## 2025-07-08 - Direct HTML string concatenation vs intermediate arrays
**Learning:** In high-frequency render functions (like `renderSearchResults` which fires on every keystroke), allocating intermediate arrays for thousands of results via `rows.push()` and then calling `rows.map().join('')` causes unnecessary memory allocation and garbage collection overhead.
**Action:** Use direct HTML string concatenation (`html += ...`) instead of array allocations (`[].push(...)` followed by `.map().join('')`) for high-frequency list rendering.
