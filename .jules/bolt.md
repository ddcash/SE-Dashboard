...
## 2025-07-18 - Intermediate array allocations in tight render loops
**Learning:** In high-frequency render functions like renderSearchResults that reconstruct the DOM via innerHTML on every keystroke, allocating intermediate arrays and using .map().join('') causes unnecessary memory allocation and GC thrashing.
**Action:** Use direct HTML string concatenation inside loops to avoid intermediate array allocations entirely.
