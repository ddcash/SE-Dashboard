...
## 2025-07-16 - Intermediate array allocations in innerHTML render strings
**Learning:** In high-frequency render functions like `renderSearchResults` that are called on every keystroke, allocating intermediate arrays (via `.push()`) and then transforming them into strings (via `.map().join('')`) causes measurable memory churn and garbage collection pauses. Direct string concatenation is much faster.
**Action:** Always accumulate HTML strings directly (e.g., `html += renderCard(...)`) rather than building arrays of objects to map and join later.
