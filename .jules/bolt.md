...
## 2025-07-07 - Inefficient O(N) array lookups for hidden items in tight render loops
**Learning:** Using `Array.includes()` inside the high-frequency `isHidden` function creates an O(N^2) bottleneck when rendering thousands of items, because the hidden array is linearly scanned for every single bookmark during `renderAllCards`.
**Action:** Cache the arrays into `Set`s based on array reference equality (`_hiddenCache.arr !== arr`), turning O(N) scans into O(1) hash map lookups.
