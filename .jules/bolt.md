...
## 2025-07-08 - Intermediate array allocation in high-frequency string rendering
**Learning:** In high-frequency render paths like search (which reconstructs the DOM on every keystroke), collecting items into an array only to map over them and `.join('')` causes unnecessary memory allocation and garbage collection thrashing.
**Action:** When generating large HTML strings in a tight loop, concatenate directly to a string accumulator instead of pushing to an array and joining it later.
