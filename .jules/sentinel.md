...
## 2024-06-03 - Fix XSS in inline onsubmit event handlers
**Vulnerability:** User-controlled inputs (such as category and bookmark IDs) were directly interpolated into inline `onsubmit` event handlers in forms (e.g., `onsubmit="submitCard(event,'${catId}','${bmId||''}')"`). Attackers can bypass standard HTML-escaping because the browser decodes HTML entities inside inline attributes before evaluating the JavaScript execution context.
**Learning:** Similar to `onclick` handlers, any inline JavaScript event handler (`onsubmit`, `oninput`, `onchange`, etc.) is vulnerable if it receives interpolated variables, even if HTML-escaped.
**Prevention:** Ensure dynamic identifiers are stored in `data-` attributes on the DOM element and accessed dynamically via `this.dataset` in the inline event handler.
