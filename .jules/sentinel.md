...
## 2025-05-18 - Fix XSS via unescaped string interpolation in inline event handlers
**Vulnerability:** Dynamic string values (such as IDs or arbitrary logic like group IDs or array indices) were being interpolated directly into inline event handlers via template literals (`onsubmit="submitGroup(event, '${groupId}')"`). Because HTML attributes decode HTML entities before parsing JavaScript, escaping the value did not prevent XSS when an attacker could craft an input containing unescaped single quotes to break out of the string context in the inline script.
**Learning:** Interpolating user-supplied strings directly into inline JS event handlers is a severe vector for Cross-Site Scripting (XSS), regardless of standard HTML escaping, because of the browser's execution order for HTML parsing and JavaScript execution in attributes.
**Prevention:** Ensure any dynamic properties are stored safely in `data-*` attributes on the element. The event handler should dynamically access these properties via `this.dataset.propertyName` or `this.getAttribute('data-property-name')`, which prevents the strings from being executed as JS.
